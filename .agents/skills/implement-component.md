...
0. The component_name should be provided in the prompt. If it isn't, ask for the name of the component to be implemented.
1. Use the context gathered from Jira and Figma to create a plan for implementing the component or feature. If there context is missing, ask for the Jira ticket number and use `/get-jira-and-figma-context [ticket_number]` command to retrieve context from Jira and Figma.
...
5. Ensure the git pre-push hook passes if there is one. It will be called `pre-push` and will usually reside in `bin` or `.husky` at the project root. If the pre-push hook fails, review the error message, address the issues, and try pushing again until it passes.
